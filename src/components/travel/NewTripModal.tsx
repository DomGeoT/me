import { Box, Button, Card, FormControl, FormLabel, IconButton, TextField, Typography } from "@mui/material";
...
export function NewTripModal({ open, onClose }: Props) {
    ...
    const handleRawMarkdownChange = React.useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    React.useEffect(() => {
        navigator.geolocation.getCurrentPosition((position: GeolocationPosition) => {
            const { longitude: _longitude, latitude: _latitude } = position.coords
    ...
    return (
        <Modal open={open} onClose={onClose}>
            <Box sx={{ width: "100%", display: 'flex', flexDirection: 'column', maxHeight: '60vh' }}> {/* Set maxHeight for dynamic sizing */}
                <Typography id="modal-modal-title" variant="h6" component="h2">
                    New Trip
                </Typography>
                <FormControl sx={{ flex: 1, overflowY: 'auto' }}> {/* Use overflowY: 'auto' to enable scrolling when needed */}
                    <Box sx={{
                        display: 'flex',
                        flexDirection: 'row',
                        "& > *": {
                        ...
                        <Box sx={{ flex: '1', pr: 4 }}>
                            ...
                            <TextField sx={{ width: '100%' }} multiline value={rawMarkdown} onChange={handleRawMarkdownChange} />
                        </Box>
                        <Box sx={{ flex: '1' }}>
